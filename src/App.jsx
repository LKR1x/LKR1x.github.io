...
// --- MODIFIED Project Modal Component ---
const ProjectModal = ({ project, onClose }) => {
  if (!project) return null;
  return (
    // FIX: Added onClick to the backdrop div. This will trigger the onClose function.
    <div
      className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 p-4 backdrop-blur-sm"
      onClick={onClose}
    >
      {/* FIX: Added onClick with stopPropagation to the content div. 
          This prevents a click inside the modal from "bubbling up" to the backdrop and closing it. */}
      <div
        className="bg-zinc-900 rounded-lg border border-zinc-700 max-w-3xl w-full max-h-[90vh] overflow-y-auto relative p-6 text-white shadow-xl modal-scroll"
...
const MainPageContent = () => {
  ...
  // FIX: Added a useEffect hook to control the body's scrollbar.
  useEffect(() => {
    // When a project is selected (modal opens), disable scrolling on the body.
    if (selectedProject) {
      document.body.style.overflow = 'hidden';
    } else {
      // When no project is selected (modal is closed), re-enable scrolling.
      document.body.style.overflow = 'auto';
    }
    // Cleanup function: This will run if the component unmounts.
    // It ensures the scrollbar is always restored.
    return () => {
      document.body.style.overflow = 'auto';
    };
  ...
  return (
    <>
      <section className="relative w-full h-svh min-h-svh bg-black text-white overflow-hidden mt-0 pt-0">
        ...
        {/* Floating Image: bottom on desktop, center on mobile */}
        <div className="absolute left-1/2 -translate-x-1/2 pb-6 z-20 animate-float
                        top-[25%] md:top-[25%] lg:top-[22%] md:bottom-0 transform -translate-y-1/2 md:-translate-y-0">
          <img
            src="/images/float.webp"
            alt="About Me"
            className="
              w-[90vw] sm:w-[75vw] md:w-[55vw] lg:w-[50vw] xl:w-[45vw] max-w-[550px] h-auto
            "
          />
        </div>

      {/* Name, tagline & nav aligned to bottom */}
      <div className="absolute bottom-24 left-0 w-full px-4 flex flex-col md:flex-row justify-between z-30">
        {/* Introduction aligned left */}
        <div className="mb-4 md:mb-0 text-left flex-1">
          <p className="text-base md:text-2xl leading-relaxed">
            ...
            CRAFTING{" "}
            <span className="font-bold bg-gradient-to-r from-pink-500 via-fuchsia-500 to-blue-500 bg-clip-text text-transparent drop-shadow-[0_0_12px_rgba(168,85,247,0.6)]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4" id="projects">
        {projects.length > 0 ? (
          ...
          <p className="text-zinc-400 col-span-full text-center">Loading projects...</p>
        )}
      </div>
      {projects.length > 6 && (
        <div className="flex justify-center mt-6 px-4">
          ...
          >
            {showAll ? "Hide projects" : "More projects"}
          </button>
        </div>
      )}
      ...
      <Link to="/playground" className="block w-full mb-16 mt-10 cursor-pointer group px-4" id="playground">
        <div
          ...
          style={{ backgroundImage: "url('/images/zz.webp')", backgroundSize: "cover", backgroundPosition: "center" }}
        >
          <div className="p-6 sm:p-8 flex flex-col justify-end h-full bg-gradient-to-t from-black via-transparent to-transparent">
            <h2 className="text-[10vw] font-bold tracking-wide gradient-text">GALLERY</h2>
            <hr className="border-zinc-700 mb-4" />
            <p className="text-lg sm:text-xl text-white">Design Experiments & Creative Snippets</p>
          </div>
        </div>
      </Link>
      <br />
        ...
        <div className="w-full max-w-5xl flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-2/3 w-full flex justify-center md:order-3 order-2">
            <form ref={form} onSubmit={sendEmail} className="w-full max-w-xl flex flex-col gap-4" >
              ...
              <button type="submit" className="animated-btn text-white font-bold py-3 px-8 rounded mt-2">Send</button>
